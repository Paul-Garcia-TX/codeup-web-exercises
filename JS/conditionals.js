"use strict";

/* ########################################################################## */

/**
 * TODO:
 * Create a function named `analyzeColor` that accepts a string that is a color
 * name as input. This function should return a message which relates to the
 * color stated in the argument of the function. For colors you do not have
 * responses written for, return a string stating so
 *
 * Example:
 *  > analyzeColor('blue') // returns "blue is the color of the sky"
 *  > analyzeColor('red') // returns "Strawberries are red"
 *
 *
 *  > analyzeColor('cyan') // returns "I don't know anything about cyan"
 *
 * You should use an if-else-if-else block to return different messages.
 *
 * Test your function by passing various string literals to it and
 * console.logging the function's return value
 */

// Don't change the next two lines!
// These lines create two variables for you:
// - `colors`: a list of the colors of the rainbow
// - `randomColor`: contains a single random color value from the list (this
//                  will contain a different color every time the page loads)
var colors = ['red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet'];
var randomColor = colors[Math.floor(Math.random() * colors.length)];
/**
alert("The random color is:  " + randomColor);
function analyzeColor(randomColor){
    if (randomColor === "red") {
            alert("Color is :" + randomColor +". Apples are Red")
    }
    else if (randomColor === "blue"){
            alert("Color is :" + randomColor +". The sky is blue.");
    }
    else if (randomColor === "orange"){
        alert("Color is :" + randomColor +". What rhymes with Orange?");
    }
    else if (randomColor === "green"){
        alert("Color is :" + randomColor +". The Hulk is green.");
    }
    else if (randomColor === "indigo"){
        alert("Color is :" + randomColor +". Blue jeans are actually indigo.");
    }
    else if (randomColor === "violet"){
        alert("Color is :" + randomColor +". Violet is a pretty color.");
    }
    else if (randomColor === "yellow"){
        alert("Color is :" + randomColor +". Yellow is a very bright color.");
    }
    else {
        alert("I don't know what happened. Wrong color I guess");
    }
}
analyzeColor(randomColor);

console.log("The color should be: " + randomColor);

*/

/**
 * TODO:
 * Pass the `randomColor` variable to your 'analyzeColor' function and console.log the results.
 * You should see a different message every time you refresh the page
 */
console.log("The color is " + randomColor );
/**
 * TODO:
 * Comment out the code above, and refactor your function to use a switch-case statement
 */
/**
 *
switch(randomColor) {
    case "red" :
        alert("Red ranger: Tyrannosaurus!");
        break;
    case "blue" :
        alert("Blue ranger: Triceratops!");
        break;
    case "yellow" :
        alert("Yellow ranger: Saber tooth Tiger!");
        break;
    case "green" :
        alert("Green ranger: Dragon Zord!");
        break;
    case "orange" :
        alert("Orange ranger: There wasn't an orange ranger. =(");
        break;
    case "violet" :
        alert("Violet ranger: There was no Violet ranger. Sad panda.");
        break;
    default:
        alert(randomColor + "Indigo? Really?");
        break;
}

let powerRanger = prompt("Which color was your favorite?");
console.log("Response was " + powerRanger);
alert("You chose the " + powerRanger+ " ranger.");

analyzeColor(powerRanger);
*/

/**
 * TODO:
 * Prompt the user for a color when the page loads, and pass the input from the
 * user to your `analyzeColor` function. Alert the return value from your
 * function to show it to the user.
 */

/* ########################################################################## */

/**
 * TODO:
 * Suppose there's a promotion in Walmart, each customer is given a randomly
 * generated "lucky number" between 0 and 5. If your lucky number is 0 you have
 * no discount, if your lucky number is 1 you'll get a 10% discount, if it's 2,
 * the discount is 25%, if it's 3, 35%, if it's 4, 50%, and if it's 5 you'll get
 * everything for free!.
 *
 * Write a function named `calculateTotal` which accepts a lucky number and total
 * amount, and returns the discounted price.
 *
 * Example:
 * calculateTotal(0, 100) // returns 100
 * calculateTotal(4, 100) // returns 50
 * calculateTotal(5, 100) // returns 0
 *
 * Test your function by passing it various values and checking for the expected
 * return value.
 */
//lucky numbers
let discountOne = .1;
let discountTwo = .25;
let discountThree = .35;
let discountFour = .50;

let beforeDiscount = prompt("What is your total?");

/**
let testPrompt = prompt("Pick a number between 1 and 5");


function calculateTotal(testPrompt){
    if (testPrompt === '1'){
        let discountAmount = beforeDiscount*discountOne;
        alert("Your discount is 10%" )
        return discountAmount;
    }
    else if (testPrompt === '2'){
        let discountAmount = beforeDiscount*discountTwo;

        alert("Your discount is 25%");
        return  discountAmount;
    }
    else if (testPrompt === '3'){
        let discountAmount = beforeDiscount*discountThree;
        alert ("Your discount is 35%");
        return discountAmount;
    }
    else if (testPrompt === '4') {
        let discountAmount = beforeDiscount*discountFour;
        alert ("Your discount is 50%");
        return discountAmount;
    }
    else if(testPrompt ==='5'){
        alert("Congratulations your purchase is free!")
    }
    else {
        alert("Please select a valid input.")
    }
}
let afterDiscount = beforeDiscount - calculateTotal(testPrompt);

alert("Your total is " + afterDiscount);
 */
/**
 * TODO:
 * Uncomment the line below to generate a random number between 0 and 5.
 * (In this line of code, 0 is inclusive, and 6 is exclusive)
 * Prompt the user for their total bill, then use your `calculateTotal` function
 * and alerts to display to the user what their lucky number was, what their
 * price before the discount was, and what their price after the discount is.
 */
// Generate a random number between 0 and 6
var luckyNumber = Math.floor(Math.random() * 6);

console.log("Number is "+ luckyNumber);
alert("Lets see if you're lucky.");

function calculateTotal(luckyNumber){
    if (luckyNumber === 1){
        let discountAmount = beforeDiscount*discountOne;
        alert("Your discount is 10%" )
        return discountAmount;
    }
    else if (luckyNumber === 2){
        let discountAmount = beforeDiscount*discountTwo;

        alert("Your discount is 25%");
        return  discountAmount;
    }
    else if (luckyNumber === 3){
        let discountAmount = beforeDiscount*discountThree;
        alert ("Your discount is 35%");
        return discountAmount;
    }
    else if (luckyNumber === 4) {
        let discountAmount = beforeDiscount*discountFour;
        alert ("Your discount is 50%");
        return discountAmount;
    }
    else if(luckyNumber === 5){
        alert("Congratulations your purchase is free!")
    }
    else if(luckyNumber === 0){
        alert("You get no discount");

    }
    else {
        alert("Please select a valid input.")
    }
}
let newTotal = beforeDiscount - calculateTotal(luckyNumber);

if (luckyNumber === 0) {
    alert("Your total is " + beforeDiscount);
}
else if (luckyNumber === 5) {
    alert("Congrats on free stuff!");
}
else {
    alert("Your total is " + newTotal);
}
/**
 * TODO:
 * Write some JavaScript that uses a `confirm` dialog to ask the user if they
 * would like to enter a number. If they click 'Ok', prompt the user for a
 * number, then use 3 separate alerts to tell the user:
 *
 * - whether the number is even or odd
 * - what the number plus 100 is
 * - if the number is negative or positive
 *
 * Do *NOT* display any of the above information
 * if the user enters a value that is not of the number data type.
 * Instead, use an alert to inform them of the incorrect input data type.
 *
 *
 * Can you refactor your code to use functions?
 * HINT: The way we prompt for a value could be improved

 so we are first going to ask user if they want to confirm

 state if number is odd or even

 what the number is pluss 100

 if number is positive or negative

 */

let yesOrNo = confirm("Would you like to enter a number?");
let aNumber;
function ask (yesOrNo){
    if (yesOrNo === true){
    console.log("The user said yes");
    alert("Ok please pick a number.");
    let aNumber = prompt("What number are you thinking of?");
    return aNumber;
    }
    else {
    console.log("The user said no");
    alert("Understandable, have a good day.");
    }
    return;
}

let returnedNumber = ask(yesOrNo);

console.log(returnedNumber);
if (isNaN(returnedNumber)) {
    alert("Sorry you didn't want to play");
}
else {

    if (returnedNumber % 2 === 0) {
        alert("The number is even");
        console.log(returnedNumber + " is even");
    } else {
        alert("The number is odd");
        console.log(returnedNumber + " is odd.");
    }
}

console.log("The number was " + returnedNumber);

let correction = parseInt(returnedNumber);

let add = correction + 100;

console.log(add);

if (correction < 0) {
    console.log("The number is negative.");
}
else {
    console.log("The number is positive");
}